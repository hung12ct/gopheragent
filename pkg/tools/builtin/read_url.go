...
func (t *ReadURLTool) Execute(ctx context.Context, argsJSON string) (tools.Result, error) {
	...
	if err != nil {
		return tools.Result{}, fmt.Errorf("tools: read_url: build request: %w", err)
	}
	...
	if resp.StatusCode != http.StatusOK {
		return tools.Result{}, fmt.Errorf("tools: read_url: server returned status %d", resp.StatusCode)
	}
	...
	if err != nil {
		return tools.Result{}, fmt.Errorf("tools: read_url: read body: %w", err)
	}
	...
	if textResult == "" {
		return tools.Text("Failed to parse meaningful text from the page (it might be a heavy JS-rendered SPA app)."), nil
	}

	return tools.Text(fmt.Sprintf("Source URL: %s\n\nContent:\n%s", url, textResult)), nil
}
