...
func TestSessionManager_SetIsolated(t *testing.T) {
	...
	sm.SetHistory(context.Background(), "s1", msgs)
...
func TestSessionManager_SaveErr(t *testing.T) {
	...
	if err := sm.Save(context.Background(), "s1"); err == nil || err.Error() != "boom" {
...
func TestSessionManager_DeleteClearsState(t *testing.T) {
	...
	if err := sm.DeleteSession(context.Background(), "s1"); err != nil {
		t.Fatalf("unexpected delete error: %v", err)
	}
	if got := sm.GetHistory(context.Background(), "s1"); got != nil {
...
func TestSessionManager_Fork(t *testing.T) {
	...
	forked := sm.GetHistory(context.Background(), newKey)
	if len(forked) != 2 {
