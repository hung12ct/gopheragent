...
// OpenAIVisionAnalyzer calls a vision-capable OpenAI model (default gpt-4o)
...
// NewOpenAIVisionAnalyzer builds an analyzer. apiKey defaults to
// OPENAI_API_KEY; model defaults to "gpt-4o" (smallest model with
// reliable vision support across all image tasks).
func NewOpenAIVisionAnalyzer(apiKey, model string) (*OpenAIVisionAnalyzer, error) {
	if apiKey == "" {
	...
	return &OpenAIVisionAnalyzer{client: openai.NewClient(apiKey), model: model}, nil
}

// Analyze sends media + prompt to the vision model and returns the response.
func (a *OpenAIVisionAnalyzer) Analyze(ctx context.Context, media, prompt string) (string, error) {
	...
	if len(resp.Choices) == 0 {
		return "", fmt.Errorf("llm: vision: no choices returned")
	}
